...
export default function App({ recipe }: Props) {
	...
	const startTimer = useCallback(
		(timer: { id: string; name: string; durationMs: number }) => {
			setActiveTimers((prev) => {
	...
	useInput((input, key) => {
		...
		if (timerSelect) {
			...
			if (key.downArrow) {
				setTimerSelectIndex((i) =>
					Math.min((currentFlat?.timers.length ?? 1) - 1, i + 1),
				);
			}
			if (key.return || input === " ") {
			...
			<Box flexDirection="column" paddingX={2} paddingY={1} width={width}>
				<Box justifyContent="space-between">
					...
					{runningTimers.length > 0 && (
						<Box>
							{runningTimers.map((t) => (
								<Text key={t.id} bold color="yellow">
									⏱ {fmtCountdown(t.startedAt + t.durationMs - Date.now())}{" "}
								</Text>
							))}
						</Box>
					)}
					...
					<>
						...
						{currentFlat.sectionIngredients.map((ing, idx) => {
							...
							const qtyStr = ing.quantities
								? ing.quantities
										.map((q) => fmtQty({ qty: q.qty, unit: q.unit }).trim())
										.join(" + ")
								: null;
							const showMass =
								mass && ing.quantities?.some((q) => shouldShowMass(q.unit));
							return (
