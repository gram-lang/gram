...
export function renderSuggestions(
	...
	for (const r of results) {
		...
		const label = r.title
			? `${chalk.bold(r.title)} ${chalk.dim(rel)}`
			: chalk.bold(rel);
