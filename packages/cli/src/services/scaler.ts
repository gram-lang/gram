 ...
 */
export function parseScaleArg(
	raw: string,
): { type: "factor"; value: number } | { type: "ref"; raw: string } {
	...
	return original
		...
		.map((i: any): ScaledItem => {
			...
			return {
				...
				originalQty: i.qty,
				scaledQty:
					typeof scaledItem?.qty === "number" ? scaledItem.qty : undefined,
			};
		});
}
