...
export function summarizeChanges(changes: FormatterChanges): string {
  ...
  if (changes.lowercasedIds > 0) parts.push(`${p(changes.lowercasedIds, 'ID')} lowercased`)
  if (changes.spacesBeforeBrace > 0) parts.push(`${p(changes.spacesBeforeBrace, 'space')} before brace removed`)
  if (changes.spacesInsideBraces > 0) parts.push(`${p(changes.spacesInsideBraces, 'brace')} interior trimmed`)
  if (changes.trailingZeros > 0) parts.push(`${p(changes.trailingZeros, 'trailing zero')} removed`)
  if (changes.temperatureSpacing > 0) parts.push(`${p(changes.temperatureSpacing, 'temperature')} spacing fixed`)
  if (changes.consecutiveBlankLines > 0) parts.push(`${p(changes.consecutiveBlankLines, 'blank line group')} collapsed`)
  if (changes.sectionSpacing > 0) parts.push(`${p(changes.sectionSpacing, 'section')} spacing normalized`)
  if (changes.trailingWhitespace > 0) parts.push(`${p(changes.trailingWhitespace, 'line')} trailing whitespace removed`)
  if (changes.eofNewline) parts.push('EOF newline added')
  return parts.join(' · ')
}

export function formatGram(source: string): { content: string; changes: FormatterChanges } {
  ...
  content = content.replace(/\{(\d+\.\d*[1-9])0+([a-zA-Z°%]*)\}/g, (_, n, u) => {
    changes.trailingZeros++
    return `{${n}${u}}`
  })
