...
function buildAliasIndex(db: Record<string, IngredientData>): Map<string, string> {
      ...
      for (const node of (step as StepAST).children) {
        if (node.type === ASTNodeType.Ingredient) {
          ...
          for (const opt of (node as AlternativeAST).options) {
            if (opt.type === ASTNodeType.Ingredient) names.push((opt as IngredientAST).name)
          }
        }
      }
    }
...
export async function suggestRecipes(
  ...
  const results = await Promise.all(
    files.map(file =>
      limit(async (): Promise<RecipeSuggestion | null> => {
        let src: string
        try { src = await readFile(file, 'utf-8') } catch { return null }

        let ast: RecipeAST
        try { ast = getAST(src) } catch { return null }
        ...
        for (let i = 0; i < withSlugs.length; i++) {
          ;(recipeIds.has(withSlugs[i]!) ? matched : missing).push(opts.withTerms[i]!)
        }

        const score = withSlugs.length > 0
  ...
  return (results.filter(Boolean) as RecipeSuggestion[])
    .sort((a, b) => b.score - a.score || (a.title ?? a.file).localeCompare(b.title ?? b.file))
    .slice(0, opts.top)
}
