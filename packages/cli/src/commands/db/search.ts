...
function matchesQuery(id: string, entry: IngredientData, query: string, exact: boolean): boolean {
  const q = query.toLowerCase()
  if (exact) {
    return slugify(id) === slugify(query)
      || entry.name?.toLowerCase() === q
      || (entry.aliases ?? []).some(a => a.toLowerCase() === q)
  }
  const haystack = [id, entry.name ?? '', ...(entry.aliases ?? [])].map(s => s.toLowerCase())
...
function renderEntry(id: string, entry: IngredientData): void {
  ...
  if (entry.aliases && entry.aliases.length > 0) {
    console.log(`  ${chalk.dim('aliases:')}  ${entry.aliases.join(chalk.dim(', '))}`)
  }
  if (entry.tags && entry.tags.length > 0) {
    console.log(`  ${chalk.dim('tags:')}     ${entry.tags.map(t => chalk.cyan(t)).join(chalk.dim(', '))}`)
  }
  ...
  if (n) {
    ...
    console.log(`  ${chalk.dim('Calories')}       ${fmt(n.calories, 'kcal', 0)}`)
    ...
    if (n.sat_fat !== undefined)  console.log(`  ${chalk.dim('  Saturated')}    ${fmt(n.sat_fat, 'g')}`)
    if (n.mono_fat !== undefined) console.log(`  ${chalk.dim('  Monounsat.')}   ${fmt(n.mono_fat, 'g')}`)
    if (n.poly_fat !== undefined) console.log(`  ${chalk.dim('  Polyunsat.')}   ${fmt(n.poly_fat, 'g')}`)
    if (n.sugar !== undefined)    console.log(`  ${chalk.dim('Sugar')}          ${fmt(n.sugar, 'g')}`)
    if (n.fiber !== undefined)    console.log(`  ${chalk.dim('Fiber')}          ${fmt(n.fiber, 'g')}`)
    if (n.sodium !== undefined)   console.log(`  ${chalk.dim('Sodium')}         ${fmt(n.sodium, 'mg')}`)
    if (n.alcohol !== undefined)  console.log(`  ${chalk.dim('Alcohol')}        ${fmt(n.alcohol, 'g')}`)
  } else {
  ...
  if (p) {
    ...
    console.log(`  ${chalk.dim('Density')}        ${fmt(p.density, 'g/mL', 3)}`)
    if (p.yield !== undefined)       console.log(`  ${chalk.dim('Yield')}          ${fmt(p.yield, '', 2).trim()}`)
    if (p.unit_weight !== undefined) console.log(`  ${chalk.dim('Unit weight')}    ${fmt(p.unit_weight, 'g')}`)
  } else {
...
export default defineCommand({
  ...
  args: {
    ...
    missing: {
      type: 'string',
      description: 'Filter entries missing a field: nutrition, physical, or aliases',
    },
    exact: {
      type: 'boolean',
  ...
  async run({ args }) {
    ...
    const entries = Object.entries(db).filter(([id, entry]) => {
      ...
      if (missingFilter === 'aliases' && (entry.aliases ?? []).length > 0) return false
      return true
    })
    ...
    console.log(chalk.dim(`  ${entries.length} result${entries.length !== 1 ? 's' : ''}`))
    console.log()
  },
})
