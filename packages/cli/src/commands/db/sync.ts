...
export default defineCommand({
	meta: {
		...
		version,
		description:
			"Add missing ingredient stubs from recipes to the database (step 1/3 — run lint then enrich after)",
	},
	args: {
		pattern: {
			type: "positional",
	...
	async run({ args }) {
		...
		const newCount =
			analysis.genuinelyNew.length + analysis.fuzzyMatches.length;
		s.stop(
