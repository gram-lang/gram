...
export default defineCommand({
	...
	args: {
		...
		"only-new": {
			type: "boolean",
			description:
				"Only add new entries, skip enrichment and conflict resolution",
			default: false,
		},
		db: {
			type: "string",
	...
	async run({ args }) {
		...
		const total =
			analysis.toAdd.length +
			analysis.toEnrich.length +
