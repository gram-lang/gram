...
export default defineCommand({
  ...
  async run({ args }) {
    ...
    const db = args['skip-db'] ? null : await loadDbSafe(config, args.db as string | undefined)
    ...
    const minMatch = Math.min(100, Math.max(0, parseInt(args['min-match'] as string, 10) || 1))
