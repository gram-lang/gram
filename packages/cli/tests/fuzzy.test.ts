...
function db(ids: string[]): Record<string, IngredientData> {
    return Object.fromEntries(ids.map(id => [id, { name: id } as IngredientData]));
}
...
describe('findSimilarInDb', () => {
    ...
    it('returns null when nothing is close enough, even for very different lengths', () => {
        const match = findSimilarInDb('a', db(['completely-unrelated-long-ingredient-name']));
...
describe('similarity (used internally by the pre-filter bound)', () => {
    it('is symmetric', () => {
        expect(similarity('kitten', 'sitting')).toBeCloseTo(similarity('sitting', 'kitten'), 5);
    });
