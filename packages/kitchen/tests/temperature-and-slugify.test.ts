...
describe('warningSeverity', () => {
    ...
    it('treats nutritional/estimation and incomplete-annotation gaps as warnings, not errors', () => {
        expect(warningSeverity[WarningCode.RELATIVE_QUANTITY_UNKNOWN_MASS]).toBe('warning');
...
describe('temperature unit validation and normalization', () => {
    it.each([
        ...
        const temp = result.sections[0].steps[0].content.find((c: any) => c?.type === 'temperature');
        expect(temp.unit).toBe(expected);
        expect(result.warnings.some(w => w.code === WarningCode.INVALID_UNIT)).toBe(false);
    });
    ...
    it('warns INVALID_UNIT for an unrecognized unit and preserves the raw value', () => {
        ...
        expect(result.warnings.some(w => w.code === WarningCode.INVALID_UNIT)).toBe(true);
    });
