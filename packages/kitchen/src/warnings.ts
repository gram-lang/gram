...
export interface WarningPayloads {
    [WarningCode.VARIABLE_NOT_FOUND]: { targetName: string; item: string; loc?: Location };
    [WarningCode.RELATIVE_QUANTITY_UNRESOLVED]: { targetName: string; item: string; loc?: Location };
    [WarningCode.RELATIVE_QUANTITY_UNKNOWN_MASS]: { targetName: string; item: string; loc?: Location };
    ...
    [WarningCode.INVALID_MODIFIER_COMBINATION]: { item: string; combination: string; loc?: Location };
...
export const warningTemplates: { [K in WarningCode]: (payload: WarningPayloads[K]) => string } = {
    ...
    [WarningCode.INVALID_BAKERS_REFERENCE]: (p) => `Cannot use '${p.item}' as the Baker's Percentage reference.`,
    [WarningCode.NO_BAKERS_REFERENCE]: () => `No Baker's Percentage reference found.`,
};
