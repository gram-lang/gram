...
export const { byLang: TIME_BY_LANG, global: TIME_GLOBAL } = compileDictionary(TIME_DICTIONARIES);

/**
 * Helper to normalize a time unit string into its canonical alias ('h', 'm', 's').
 */
export const resolveTimeUnit = (unit?: string | null, lang?: string): string => {
    if (!unit) return '';
