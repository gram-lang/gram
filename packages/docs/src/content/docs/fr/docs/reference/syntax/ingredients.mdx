...
:::note
Pour un guide complet sur l'utilisation de cette fonctionnalité, consultez le guide sur la [Mise à l'échelle dynamique des Recettes](../../how-to/scale-recipes.mdx#pourcentage-du-boulanger-bakers-math).
:::
...
Attention, la syntaxe `:` ne fait que maquiller l'*affichage* de l'ingrédient dans les étapes lues par l'utilisateur : elle n'a strictement aucune incidence sur la consolidation de la liste de courses. Cette dernière (tout comme l'ID canonique de l'ingrédient) se base exclusivement sur le **vrai nom** (celui avant les deux-points), lequel est ensuite confronté aux `aliases` de la base `ingredients.yaml` (un mécanisme distinct servant à fusionner les synonymes inter-recettes, cf. [Gérer une Base de Données d'Ingrédients](../../how-to/manage-database.md)). Par conséquent, déclarer `@jaunes d'œufs:œufs{2}` et `@blancs d'œufs:œufs{4}` ne les fusionnera **jamais** dans votre liste de courses, malgré leur alias d'affichage commun `œufs` — leurs vrais noms diffèrent.
:::
