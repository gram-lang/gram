...
```gram title="Variantes de la directive @use"
...
Si une recette A importe B et que B tente d'importer A (directement ou indirectement), Gram détecte la dépendance circulaire, interrompt uniquement cette branche d'import et signale une erreur `MODULE_CYCLE` affichant la boucle complète (`A -> B -> A`). Le reste de la recette continue de se compiler — la liaison cyclique reste simplement non résolue plutôt que de faire échouer toute la compilation — mais `gram check` (et tout run `--strict`) échoue dessus, `MODULE_CYCLE` étant de sévérité "erreur".
:::
