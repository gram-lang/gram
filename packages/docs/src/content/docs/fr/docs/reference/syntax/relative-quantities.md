...
Puisque la valeur d'une quantité relative dérive par nature d'un autre ingrédient, elle ne peut logiquement pas servir de [cible de référence pour `--scale`](/fr/docs/how-to/scale-recipes#les-limites-de-lajustement-par-ingrédient-repère) ni de base de % du Boulanger. Consultez le [Guide détaillé : Ajustement des proportions](/fr/docs/explanation/scaling) pour plonger dans les règles et codes d'erreur qui régissent ces limitations.
