...
<Steps>
1. **Structure** — Identifier les phases distinctes (pâte, garniture, sauce, montage…). Chaque phase avec sa propre liste d'ingrédients est une candidate pour une `## Section`. Tout ce qui doit être préparé à l'avance reçoit un délai de rétroplanning (voir [Structure du Document](./document-structure.md#rétroplanning-ordonnancement)).
...
```
Un `->&nom` au niveau d'une section capture déjà la sortie de toutes les étapes qu'elle contient — voir [Variables Intermédiaires](./intermediate-variables.md#déclaration-au-niveau-de-la-section).
