...
- `--nutrition <auto|total|per-portion|per-100g>` <Since v="1.1.0" /> choisit la base nutritionnelle affichée (par défaut `auto` : par portion quand la recette déclare `portions:`, sinon toute la recette). `per-100g` est rapporté pour 100 g du mélange brut assemblé — voir [Estimation nutritionnelle](/fr/docs/explanation/nutrition).
...
- **Vidéos et Shorts YouTube** <Since v="1.1.0" /> : analyse directement la vidéo grâce aux capacités multimodales de Gemini (sans passer par des scrapers de sous-titres tiers).
...
**Spécificités de l'import vidéo YouTube :** <Since v="1.1.0" />
...
- Options : `--output/-o <fichier>`, `--yes/-y`, `--force`, `--max-duration <minutes>` <Since v="1.1.0" />, ainsi que les [options IA communes](#choisir-le-modèle-ia).
