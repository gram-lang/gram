...
Quand `--provider` désigne un fournisseur autre que celui de votre configuration, les champs `model`, `apiKey` et `baseUrl` écrits sous `ai:` ne sont **pas** repris : ils appartiennent au fournisseur pour lequel ils ont été écrits. Gram réclamera la variable d'environnement propre au nouveau fournisseur.
