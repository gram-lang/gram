...
- **Vidéos et Shorts YouTube** : analyse directement la vidéo grâce aux capacités multimodales de Gemini (sans passer par des scrapers de sous-titres tiers).
...
- `source:` et `author:` proviennent des données source (ou de YouTube), ou sont supprimés si elles n'en contiennent pas — jamais laissés à l'invention du modèle.
...
**Spécificités de l'import vidéo YouTube :**
- **Google / Gemini uniquement** : Gemini est le seul fournisseur capable de lire nativement une URL YouTube. Avec un autre fournisseur, l'import s'interrompt pour éviter d'envoyer une URL au lieu d'une vidéo.
- **Le coût est proportionnel à la durée**, à environ 100 tokens d'entrée par seconde de vidéo : un Short représente quelques milliers de tokens, dix minutes environ 60 000, vingt-six minutes environ 158 000. Au-delà de 20 minutes, la vidéo est refusée par défaut ; relevez le plafond avec `--max-duration <minutes>`.
- **L'estimation de coût nécessite `YOUTUBE_API_KEY`** : une simple clé YouTube Data API, lue uniquement dans les variables d'environnement (jamais dans `config.yaml`). Sans elle l'import fonctionne quand même, mais il ne peut ni déterminer la durée préalable ni appliquer le plafond — et il le signale.
- **Estimation des quantités en vidéo** : une vidéo énonce beaucoup moins de quantités qu'une recette écrite. Le prompt demande au modèle de laisser `{}` vide plutôt que de deviner, et les contrôles ci-dessus refusent ensuite tout résultat où du contenu a disparu. Attendez-vous à compléter des trous à la main : c'est le résultat honnête, pas un échec.
- Le titre, la chaîne et l'URL proviennent directement de YouTube et alimentent `title:`, `author:` et `source:`.
