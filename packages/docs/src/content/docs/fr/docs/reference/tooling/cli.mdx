...
3. #### `gram db enrich`
   Lâche l'IA sur votre base de données pour boucher les trous et auto-compléter les données manquantes.
   ...
   - Idempotent : la commande peut être relancée sans crainte, elle n'hydratera que les champs encore vides.
...
- Une entrée viciée ne fait pas crasher l'outil : chaque ligne est validée indépendamment. Le rapport final recrachera la liste exhaustive des problèmes en une seule passe.
...
1. **Variables d'Environnement** : Les variables comme `GEMINI_API_KEY` (du système ou d'un `.env`) ont toujours le dernier mot. C'est l'approche **recommandée** pour sécuriser des secrets en local comme en CI/CD.
