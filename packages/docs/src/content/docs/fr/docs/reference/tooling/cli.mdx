...
1. #### `gram db sync [motif]`
   Parcourt vos recettes pour trouver des ingrédients non documentés et les ajoute à votre base de données.
...
2. #### `gram db lint`
   Utilise l'IA pour détecter et résoudre les doublons sémantiques et les pluriels dans votre base de données.
...
3. #### `gram db enrich`
   Utilise l'IA pour compléter automatiquement les données manquantes dans votre base de données.
