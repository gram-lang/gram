...
3. #### `gram db enrich`
   ...
   - Options : `--ingredient <slug>` (enrichir une seule entrée), `--field density|nutrition|tags|category|all` (défaut `all`), `--yes/-y` (ignorer la revue et tout accepter automatiquement — pour un usage scripté), `--report/-r` (aperçu de ce qui a besoin d'être revu, sans rien écrire ; `--dry-run/-n` est un synonyme), `--db <chemin>`.
</Steps>
