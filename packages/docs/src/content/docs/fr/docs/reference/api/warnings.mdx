...
Une recette malformée ou incomplète finit quand même de compiler : le compilateur et l'Analyseur collectent des objets `Warning` structurés au lieu de *throw* des exceptions. Cela permet aux appelants d'afficher la recette tout en signalant visuellement ce qui coince. L'énumération `WarningCode` et ses utilitaires sont exportés par `@gram-lang/kitchen`.
