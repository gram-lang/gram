...
1. ### Récupérer le JSON
   ...
   Si vous omettez `db`, l'étape d'analyse est totalement ignorée (`analyzed` vaudra `null`). Jetez un œil à la [référence de l'API de `@gram-lang/cli`](/fr/docs/reference/api/#raccourci-optionnel-node-uniquement-gram-langcli) pour découvrir la liste exhaustive des options (facteur d'échelle, % du boulanger, etc.).
   :::
