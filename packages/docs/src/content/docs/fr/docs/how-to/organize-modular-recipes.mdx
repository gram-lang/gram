...
Si la recette A importe B et que B tente d'importer A, Gram détecte immédiatement la boucle et interrompt la compilation avec une erreur explicite `MODULE_CYCLE` affichant la chaîne en cause (`A -> B -> A`).
