...
description: "Générez une liste de courses unique et consolidée à partir de plusieurs recettes .gram avec la commande CLI gram shop."
---
...
:::note
Lors de la mise à l'échelle de plusieurs fichiers à la fois, vous devez utiliser un multiplicateur numérique (`--scale 2`). Vous ne pouvez pas faire une mise à l'échelle par référence d'ingrédient (`--scale farine=500g`), car le compilateur ne saurait pas la farine de *quelle* recette cibler.
:::
