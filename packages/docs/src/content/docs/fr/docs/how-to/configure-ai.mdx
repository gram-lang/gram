...
description: "Découvrez comment configurer les fournisseurs d'IA (Google Gemini, OpenAI, Anthropic, Ollama) pour importer des recettes, nettoyer la base d'ingrédients et enrichir automatiquement les données dans le CLI Gram."
---
...
3. **Enrichissement automatique (`gram db enrich`)** : Recherche et complète les valeurs nutritionnelles (calories, macros, micronutriments) et les caractéristiques physiques (densité, rendement, poids unitaire) des ingrédients incomplets de votre base.
...
- **`gram db enrich`** : L'IA recherche et complète les densités physiques (conversion volume/masse) et les valeurs nutritionnelles standards sans recherche manuelle fastidieuse.
...
1. **Tester l'enrichissement (mode simulation)** :
   ```bash
   gram db enrich --dry-run
   ```
   Si l'IA est correctement configurée, la commande analysera la base sans retourner d'erreur.
