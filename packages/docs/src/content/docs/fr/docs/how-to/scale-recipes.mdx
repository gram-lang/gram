...
Si vous ajustez les proportions de cette recette avec un facteur 2 (via `gram build --scale 2`), tout est bêtement doublé. Et si vos placards sont vides et qu'il ne vous reste que `400 g` de farine, vous pouvez utiliser l'ajustement par ingrédient repère : `gram build --scale farine=400g`. Gram calculera le multiplicateur (`0.8`) et ajustera tout automatiquement.
