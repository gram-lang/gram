...
description: "Synchronisez, nettoyez et enrichissez votre base ingredients.yaml avec le CLI Gram pour la nutrition et les calculs de masse."
---
...
1. ### Synchroniser les Nouveaux Ingrédients
   ...
   :::tip
   Le CLI utilise une recherche approximative (fuzzy matching) pour vous suggérer d'éviter les doublons s'il détecte une faute de frappe !
   :::
...
2. ### Nettoyage et Déduplication (Linting)
   ...
   :::tip
   L'IA respecte la langue par défaut définie dans votre fichier de configuration `config.yaml`. Si elle fusionne des termes anglais et français, elle vous proposera intelligemment le terme dans votre langue préférée comme clé principale !
   :::
