...
2. ### Nettoyage et déduplication (*Linting*)
   ...
   La liste `aliases` sert exclusivement à l'agrégation de la liste de courses pour regrouper les ingrédients entre différentes recettes. Elle n'a aucun rapport avec le renommage ponctuel `@Vrai Nom:Nom d'Affichage` (voir [Syntaxe des ingrédients](../reference/syntax/ingredients.md)), qui modifie uniquement l'affichage d'un ingrédient dans le texte des étapes, sans jamais affecter le regroupement ou la liste de courses.
   :::
