...
La mise à l'échelle inversée paraît simple sur le papier — il suffit de diviser la quantité cible par la quantité actuelle de l'ingrédient. Pourtant, une implémentation naïve se trompe silencieusement sur un nombre surprenant de cas :
...
- Si un ingrédient est utilisé avec deux unités incompatibles dans la même recette, seule sa quantité "primaire" figure dans la liste de courses agrégée. En déduire un facteur d'échelle reviendrait à ignorer le reste silencieusement.
