...
1. ### Analyse Syntaxique (`@gram-lang/parser`)
   Le voyage démarre ici. Le *parser* avale une chaîne de caractères brute (votre fichier `.gram`) et s'appuie sur une grammaire OhmJS pour en éprouver la syntaxe. 
   Si tout est *safe*, il accouche d'un **Arbre Syntaxique Abstrait (AST)**. Cet AST est dénué de toute logique métier : c'est une simple représentation structurelle du texte.

2. ### Compilation (`@gram-lang/kitchen`)
   L'AST est ensuite balancé à la Kitchen (*Cuisine*). Elle porte la responsabilité de la logique structurelle de la recette :
   - **Scope (*Scoping*)** : Résolution des variables (ex : câbler une référence `&pâte` sur sa déclaration `->&pâte`).
   ...
   Le résultat ? Une recette logiquement irréprochable et compilée sous la forme d'un `CompilationResult`.
...
4. ### Présentation (`@gram-lang/renderer` ou personnalisé)
   Votre recette gorgée de données est prête. Vous pouvez dégainer `@gram-lang/renderer` pour cracher du Markdown, une UI web standard, ou un HTML statique taillé pour l'impression. Ou bien, *by-passez* tout ça et injectez directement le JSON final dans votre front-end React, Vue ou app native.
</Steps>
