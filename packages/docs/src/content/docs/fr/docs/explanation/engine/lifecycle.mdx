...
2. ### Résolution & Composition de modules (`@gram-lang/modules`)
   Si la recette fait appel à des directives `@use`, `@gram-lang/modules` parcourt le graphe de dépendances (détection de cycles, calcul des rendements des sous-bases et isolation des variables intermédiaires) et fusionne les modules en un **AST composé** unique et autonome. Une recette sans import passe cette étape inchangée.
