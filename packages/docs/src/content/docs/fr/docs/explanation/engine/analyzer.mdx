...
Le *package* `@gram-lang/analyzer` incarne l'étape analytique finale du pipeline de compilation. Il récupère le `CompilationResult` (logiquement valide) craché par `@gram-lang/kitchen` et procède à un **Enrichissement Physique** en croisant la recette avec votre base de données `ingredients.yaml`.
...
1. **Masse Physique** : Si l'unité est déjà un poids (`g`, `kg`, `oz`, `mg`, `lb`), elle est bêtement convertie en grammes.
...
Sans densité disponible pour un volume, l'analyseur **refuse** de faire la supposition aveugle qu'il s'agit d'eau. Il préfère cracher un *warning* « aucune masse » plutôt que de risquer une agrégation bancale (traiter "1 tasse de farine" comme de l'eau ruinerait une recette). L'ingrédient reste listé dans son unité brute.
:::
...
- Garde prudemment des entrées séparées — estampillées de l'ID canonique avec un flag `multiUnit: true` — si la masse est insoluble (ex: manque de densité), refusant là encore de jouer aux devinettes.
