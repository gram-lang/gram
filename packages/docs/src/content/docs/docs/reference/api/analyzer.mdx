...
description: "Enrich compiled recipes with standardized mass, purchasing yield, nutrition estimates, and baker's percentages using an ingredient database."
---
...
`analyze()` is pure and never mutates `result`. `AnalyzedCompilationResult` extends `CompilationResult` — same shape, plus per-usage `normalizedMass`/`conversionMethod`/`isEstimate`/`purchasingMass`/`bakersPercentage` fields and a `metrics.nutrition` block. See [Data Formats](/docs/reference/api/data-formats) for a fully annotated example.
...
Validates entry-by-entry rather than all-or-nothing: one malformed ingredient doesn't prevent every other valid one from loading. Use `data` to compile/analyze; surface `rejected` to the user (e.g. `gram db validate`). See [Data Formats](/docs/reference/api/data-formats) for the `IngredientData` YAML schema.
...
<GeneratedTable table={unitConversionsTable("en")} />
...
Every basis is derived from the same unrounded sums and rounded independently, so a per-portion figure never compounds the rounding of the total. `basis` is the denominator behind `per100g` — see [Nutritional Estimation](/docs/explanation/nutrition) for why it matters and what "per 100 g" means when no cooking loss is modelled.
