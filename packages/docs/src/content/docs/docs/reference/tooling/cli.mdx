...
- `source:` and `author:` are taken from the source data, or removed when it has none — never left for the model to invent.
...
##### Importing from a YouTube video

```bash
gram import "https://www.youtube.com/watch?v=…"
gram import "https://www.youtube.com/shorts/…"     # normalised automatically
```

The video is watched directly by Gemini — there is no transcript step, because no official API gives you the captions of a video you don't own. Consequences worth knowing:

- **Google only.** Gemini is the one provider that reads a YouTube URL. With any other provider the import stops rather than sending it something that isn't a video.
- **Cost scales with length**, at roughly 100 input tokens per second of video: a Short is a few thousand tokens, ten minutes is around 60,000, twenty-six minutes around 158,000. Videos over 20 minutes are refused; raise the ceiling with `--max-duration`.
- **The cost estimate needs `YOUTUBE_API_KEY`** (a plain YouTube Data API key, read from the environment only, never from `config.yaml`). Without it the import still runs, but it cannot tell you the length or enforce the limit, and says so.
- **A video states far fewer quantities than a written recipe.** The prompt tells the model to leave `{}` empty rather than guess, and the checks above then refuse anything where content went missing. Expect gaps to fill in by hand — that is the honest outcome, not a failure.
- Title, channel and URL come from YouTube itself and are used verbatim for `title:`, `author:` and `source:`.
...
When `--provider` names a provider other than the one in your config, the `model`, `apiKey` and `baseUrl` written under `ai:` are **not** carried over — they belong to the provider they were written for. You will be asked for that provider's own environment variable instead.
