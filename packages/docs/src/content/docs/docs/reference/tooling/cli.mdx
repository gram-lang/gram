...
3. #### `gram db enrich`
   ...
   - Options: `--ingredient <slug>` (enrich a single entry), `--field density|nutrition|tags|category|all` (default `all`), `--yes/-y` (skip the review, accept everything — for scripting), `--report/-r` (preview what needs review without writing; `--dry-run/-n` is a synonym), `--db <path>`.
</Steps>
