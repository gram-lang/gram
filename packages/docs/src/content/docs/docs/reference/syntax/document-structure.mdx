...
*   `yields`: (String) How much this recipe produces, read only when another recipe [imports](/docs/reference/syntax/modules) it — e.g. `yields: 500g`, `yields: 24 cookies`. Determines the scale factor an importer's `&binding{qty}` request is measured against. Without it, Gram measures the recipe's own mass instead.
