---
title: "Times & scheduling"
description: "Define ~timer durations and units in your recipes, and how Gram schedules active versus passive time."
---
...
:::note
`m` or `minutes` will be automatically corrected to `min` by the compiler.
:::
...
For a more detailed explanation of how the timeline is optimized, check out the [ALAP Scheduling Deep Dive](/docs/explanation/alap-scheduling).
...
See also: [Retro-planning (Scheduling)](/docs/reference/syntax/document-structure#retro-planning-scheduling) in the document structure reference.
