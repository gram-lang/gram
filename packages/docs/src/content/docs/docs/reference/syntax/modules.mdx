...
If Recipe A imports B which imports A (directly or indirectly), Gram detects the circular dependency and halts compilation immediately with a `MODULE_CYCLE` diagnostic displaying the full dependency loop (`A -> B -> A`).
:::
