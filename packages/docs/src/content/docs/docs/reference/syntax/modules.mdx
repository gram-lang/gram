...
<Tabs>
  ...
  <TabItem label="Multi-word names">
    ...
    ```gram
    @use "./bases/puff-pastry.gram" as &puff pastry{}
    @use "./bases/puff-pastry.gram" as { &water dough{} as &detrempe }
    ```
  </TabItem>
...
```gram title="bases/inverted-puff-pastry.gram"
---
...
[Mix] @flour{250g}, @water{120ml}, @salt{5g}, and @butter{50g}(melted). Chill for ~_{2h}.

## Tourage Butter Block ->&beurre-manie
[Knead] @butter{300g}(cold) with @flour{100g} into a block. Chill for ~_{1h}.
```
...
Declare the module's baseline output using the `yields:` frontmatter key:
...
When an importing recipe requests `&pate{250g}`, Gram scales every ingredient in `shortcrust.gram` by `0.5` automatically.

### Yield Formats
* **Mass or Volume**: `yields: 500g`, `yields: 1L`
* **Discrete Count**: `yields: 24 cookies`, `yields: 1 tart`
* **Multi-component map**: `yields: [detrempe:350g, beurre-manie:400g]`

:::note[Fallback Mass Standardization]
If `yields:` is omitted, Gram computes the module's net mass from its ingredients. If that calculation depends on estimated densities, Gram emits an `ESTIMATED_MODULE_YIELD` diagnostic.
:::

### Scaling Destructured Exports
When importing multiple components from the same module, each component is measured against its declared yield. If different ratios are requested, the **largest ratio** applies to the entire module, and Gram reports the extra production:

```gram
@use "./bases/elements-feuilletage.gram" as { &detrempe, &beurre-manie }

[Roll] &detrempe{700g} and &beurre-manie{400g}.
```

If `elements-feuilletage.gram` yields 350g of détrempe and 400g of beurre manié, requesting `&detrempe{700g}` scales the module ×2 (producing 800g of beurre manié). Gram emits `MODULE_SURPLUS` to inform you of the surplus.

### Timers & Batches
**Cooking and resting times are never scaled.** Doubling a cookie dough does not double or halve the baking duration. 

When a discrete count is requested against a mass yield (`&cookies{2}` against `yields: 500g`), Gram interprets the request as *2 full batches* and emits `MODULE_BATCH_INTERPRETATION`.
...
* `yields:` is consumed to calculate scaling factors, then discarded.
...
<CardGrid>
  <LinkCard 
    ...
    description="Step-by-step guide to extracting and importing your first reusable dough." 
    href="/docs/tutorials/module-imports" 
  />
  <LinkCard 
    title="How-To: Organize Modular Recipes" 
    description="Best practices for project structure, multi-day scheduling, and --stock workflows." 
