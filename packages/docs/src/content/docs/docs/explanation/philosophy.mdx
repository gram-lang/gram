---
title: "What is Gram?"
description: "Why Gram treats recipes as code: the developer-first principles behind deterministic cooking logic."
---

import { CodeTabs } from 'starlight-theme-nova/components';

Gram is an open-source declarative DSL (*Domain-Specific Language*) for cooking recipes, built for developers.

Most recipe formats are just static blocks of text. Gram takes a different approach: it takes your instructions written in plain text and compiles them into structured, calculated, and relational data.
...
<CardGrid>
  ...
  <Card title="Language Server (LSP)" icon="laptop">
    **`@gram-lang/language-server`**: Brings proper editor support to recipes (autocompletion, diagnostics, hover tooltips).
  </Card>
