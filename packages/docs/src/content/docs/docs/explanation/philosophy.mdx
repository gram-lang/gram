...
Gram is an open-source declarative, computational recipe DSL (Domain-Specific Language) built for developers and food lovers.
...
- **Modularity & Reusability (`@use`)**: Split shared bases (pastries, stocks, sauces) into their own `.gram` files. Gram seamlessly inlines their steps, automatically scales their quantities to match what your dish asks for (`yields:`), and merges their raw ingredients onto your unified shopping list.
