...
## Scaling Modular Imports (`@gram-lang/modules`)

When a recipe imports an external base via `@use` (such as `@use "./bases/crust.gram" as &crust`), scaling follows a yield-based evaluation:

1. **Yield Measurement**: `@gram-lang/modules` pre-analyzes the sub-recipe to compute its total mass (`resolveYield`), recursively summing all contributing sections.
2. **Ratio Computation**: When the host recipe specifies a reference amount (e.g. `&crust{300g}` against a base that yields `400g`), `computeScaleFactor()` computes a factor of `300 / 400 = 0.75`.
3. **AST Pre-Scaling**: The sub-recipe's AST is scaled (`scaleAst`) *before* being spliced into the host recipe. If multiple steps request different amounts of the same bound export, the requests are summed; if multiple exports are bound, the maximum ratio wins.
