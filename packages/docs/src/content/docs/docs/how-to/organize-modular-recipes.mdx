...
For example, poaching a whole chicken with aromatics yields both tender shredded meat and a rich golden broth:

```gram title="components/bases/poached-chicken.gram"
---
...
[Poach] The @whole chicken{1.5kg} with @onion{1}, @carrots{2}, and @celery stalks{2} in water for ~{45min}.

[Shred] Once cooled.

## Golden Broth ->&broth

[Simmer] The cooking liquid for ~{15min} to concentrate flavor.
```

In your main recipe, destructured imports let you bind both components cleanly:

```gram title="chicken-pot-pie.gram"
@use "@/components/bases/poached-chicken.gram" as { &meat, &broth }

## Pot Pie Filling

[Make] A creamy velouté sauce using the &broth{400ml}, @butter{40g}, and @flour{40g}.

[Fold] In the shredded &meat and @frozen peas{100g}, then bake under #puff pastry{}.
```

:::tip[Local renaming]
If a module's export names clash with variables in your own recipe, rename them with `as`:
```gram
@use "@/components/bases/poached-chicken.gram" as {
  &meat as &chicken filling{},
  &broth as &pie gravy{}
}
```
:::
