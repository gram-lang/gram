...
:::tip[Community starter databases]
The [`community-db/`](https://git.gram-lang.org/gram-lang/gram/src/branch/main/community-db) directory in the Gram repository collects community-contributed starter files by language/cuisine, ready to `gram db merge`. They're opt-in starting points, not a verified reference — see [Data Philosophy](/docs/explanation/philosophy#data-philosophy-why-no-external-nutrition-database) for why Gram doesn't ship one built in.
:::
