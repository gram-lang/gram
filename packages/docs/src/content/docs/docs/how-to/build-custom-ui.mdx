...
1. ### Getting the JSON data
   ...
   Omit `db` to skip the analyzer stage entirely (`analyzed` comes back `null`). See [`@gram-lang/cli`'s API reference](/docs/reference/api/#optional-node-only-shortcut-gram-langcli) for the full option list (scale factor, baker's percentages).
   :::
