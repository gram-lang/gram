...
description: "Learn how to configure AI providers (Google Gemini, OpenAI, Anthropic, Ollama) for recipe importing, ingredient database linting, and automated data enrichment in the Gram CLI."
---
...
3. **Database Enrichment (`gram db enrich`)**: Automatically queries nutritional values (calories, macros, micronutrients) and physical properties (densities, yields, unit weights) to fill in incomplete entries in `ingredients.yaml`.
...
- **`gram db enrich`**: AI fetches standardized physical densities (volume-to-mass conversion) and nutritional facts from culinary domain knowledge without requiring manual lookup.
...
1. **Test Ingredient Enrichment (Dry Run)**:
   ```bash
   gram db enrich --dry-run
   ```
   If AI is correctly configured, the command will analyze incomplete ingredients without error.
