...
description: "A step-by-step tutorial writing a complete Lemon Tart recipe using Gram's advanced compiler features."
---
...
[Rest] Wrap in #plastic wrap{} and let it rest in the fridge for ~{1h}.
```
...
4. **[Rest]** Wrap in plastic wrap and let it rest in the fridge for ⏲️ 1 h.
```
</CodeTabs>
...
In Step 1, the following was written: `[Rest] Wrap in #plastic wrap{} and let it rest in the fridge for ~{1h}`. 
...
```
```markdown title="Preview"
1. **[Rest]** Wrap in plastic wrap and let it rest in the fridge for ⏳ 1 h (passive).
```
</CodeTabs>
...
:::note
For a deep dive into how Gram handles complex relative calculations (like using volumes or units as targets), check out the [Relative Quantities](/docs/reference/syntax/relative-quantities) reference.
:::
...
2. **[Crumble]** Add the **butter** (115 g) (cold, cut into small cubes) and mix for ⏲️ 1-2 min until sandy.
3. **[Combine]** Add the **egg** (1), **vanilla extract** (1/2 tsp) (optional) and mix until the dough comes together. 
4. **[Rest]** Wrap in plastic wrap and let it rest in the fridge for ⏳ 1 h (passive).
...
Now that you've written a complete single-file recipe, explore how to organize multi-file collections and master Gram's syntax:

<CardGrid>
  <LinkCard 
    title="How-To: Organize Modular Recipes" 
    description="Learn how to extract reusable bases with @use, manage project folders, and configure aliases." 
    href="/docs/how-to/organize-modular-recipes" 
  />
  <LinkCard 
    title="Language Reference" 
    description="Explore the complete grammar specification for ingredients, cookware, timers, and variables." 
