...
4.  **Analysis [`@gram/analyzer`]:** The compiled JSON is enriched with physical properties (mass standardization, yield percentages, portion scaling, and macronutrient estimations) by querying an external database.
...
*   **Mass Standardization**: Resolves count/volume quantities to grams (`g`) using density and unit weight mappings from the provided database and YAML metadata overrides.
