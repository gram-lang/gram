...
- **Smart Escaping (`toHTML`/`toPrintHTML` only)**: These two formatters escape HTML entities in user-generated recipe content (titles, ingredient names, step text, comments) before embedding it, so it's safe to insert their output into a page as-is.
  > [!WARNING]
  > **`toMarkdown` does not escape anything.** Standard Markdown renderers (`markdown-it`, `remark`, etc.) pass raw HTML through by default, so if a recipe's title, ingredient name, or step text contains something like `<img src=x onerror=...>`, it will survive `toMarkdown()` unescaped and can execute as script once rendered to HTML downstream — a real risk if you ever render imported/shared `.gram` files (e.g. via `gram import`) rather than only ones you wrote yourself. If you render `toMarkdown()`'s output to HTML, sanitize it first (e.g. `rehype-sanitize`, `markdown-it` with `html: false`, or DOMPurify on the final HTML) — don't rely on Gram's Markdown output being pre-sanitized.
