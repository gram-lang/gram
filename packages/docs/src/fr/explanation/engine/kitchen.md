...
- **Diagnostics** : Le processeur a pour mission d'attraper les erreurs logiques, mais tous les diagnostics n'ont pas le même poids — une carte `warningSeverity` partagée (également utilisée par `gram check` et le serveur de langage) décide lesquels sont bloquants. Si vous référencez `&pâte` sans jamais l'avoir déclarée, il lève `UNDEFINED_REFERENCE` — une `error` structurelle, puisque rien en aval ne peut la résoudre. Il signale aussi `CIRCULAR_REFERENCE` (même sévérité `error`) si un ingrédient essaie d'être un pourcentage de lui-même directement — les cycles indirects à travers l'ensemble de la recette sont détectés séparément par `graph.ts` et remontés via `isCircular` sur l'article concerné de la liste de courses. Si une quantité relative (ex : `50 % &farine`) pointe vers un ingrédient qui n'est pas présent dans la même section, il lève `RELATIVE_QUANTITY_UNRESOLVED` (référence fantôme) — seulement un `warning`, car c'est une estimation incomplète mais récupérable, pas une rupture structurelle. `gram check --strict` promeut chaque `warning` en `error`.
- **Génération de la Chronologie (Ordonnancement ALAP)** : Le moteur utilise un algorithme d'ordonnancement **ALAP (As Late As Possible)** à travers une compilation en trois phases pour générer une chronologie d'exécution hautement optimisée.
  - **Phase 1 (Forward Pass)** : Le compilateur estime les durées et les tâches de fond passives pour déterminer le temps nécessaire à la production des préparations intermédiaires (`->&nom`).
  - **Phase 2 (Backward Pass)** : Il parcourt la recette à l'envers, de la fin vers le début. Lorsqu'une étape consomme un intermédiaire (`&nom`), le moteur enregistre cette dépendance pour trouver l'heure limite la plus tardive à laquelle il doit être prêt. L'étape qui produit l'intermédiaire est alors ordonnancée *juste-à-temps* pour se terminer exactement au moment voulu, évitant ainsi que les ingrédients ne reposent trop longtemps.
  - **Phase 3 (Global Commit)** : Enfin, le moteur valide la chronologie et résout les minuteurs de fond séquentiels (ex. `Named Tracks`), garantissant que le Temps Total reflète un diagramme de Gantt réaliste et optimisé, sans temps morts inutiles.
