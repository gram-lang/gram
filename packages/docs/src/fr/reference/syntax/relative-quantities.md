...
Puisque la valeur d'une quantité relative est toujours dérivée d'un autre ingrédient, elle ne peut pas non plus être utilisée comme [cible de référence pour `--scale`](/fr/how-to/scale-recipes#limits-of-reference-scaling) ou comme base de Pourcentage du Boulanger — voir le [Guide détaillé : Mise à l'échelle](/fr/explanation/scaling) pour l'ensemble complet des règles et codes d'erreur partagés entre les deux.
