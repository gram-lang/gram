...
### 1. Basic Ingredients
* **Simple**: `Add the @butter.` (Braces `{}` are optional for single words without quantities).
* **Quantity**: `Add @butter{100}.`
* **Unit**: `Add @butter{100g}.`
* **Compound Name**: `Add @sea salt{}.` (Braces mandatory if the name contains spaces).
...
* **Composites** (Sub-parts):
    * Source: `@zest<@lemon` (Cost: 1), `@juice{2}<@lemon` (Cost: 2). No spaces allowed around `<`.
...
* **Retro-planning**:
    * *Definition*: At the end of the title `## Title ~{-2d}` (Can be written before or after `->&dough`).
