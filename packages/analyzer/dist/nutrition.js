...
function calculateNutrition(ingredients, database, portions = 1) {
    const total = {
        calories: 0,
        protein: 0,
        carbs: 0,
    ...
    flatList.forEach(item => {
        ...
        else if (item.qty && (typeof item.qty === 'number' || item.qty.value)) {
            const val = typeof item.qty === 'number' ? item.qty : item.qty.value;
            const unit = item.unit || 'unit';
            const norm = (0, mass_normalization_1.normalizeMass)(val, unit, database, item.name);
            if (norm) {
                mass = norm.mass;
        ...
        if (mass > 0) {
            const data = (0, ingredient_db_1.getIngredientData)(id, database);
            if (!data) {
