...
const database: Record<string, IngredientData> = {
    flour: { name: 'Flour', nutrition: { calories: 364, protein: 10, carbs: 76, fat: 1 } },
    sugar: { name: 'Sugar', nutrition: { calories: 400, protein: 0, carbs: 100, fat: 0, sugar: 100 } },
    'lemon-juice': { name: 'Lemon Juice', nutrition: { calories: 22, protein: 0.4, carbs: 6.9, fat: 0.2 } },
    'lemon-zest': { name: 'Lemon Zest' }, // no nutrition block on purpose
    'whipped-cream': { name: 'Whipped Cream', nutrition: { calories: 257, protein: 2.1, carbs: 3.4, fat: 26 } },
    butter: { name: 'Butter', nutrition: { calories: 717, protein: 0.9, carbs: 0.1, fat: 81 } },
    oil: { name: 'Oil', nutrition: { calories: 884, protein: 0, carbs: 0, fat: 100 } },
};
...
describe('calculateNutrition — optional ingredients', () => {
    it('excludes a top-level optional ingredient from totals and coverage', () => {
        const withOptional = calculateNutrition(
            [
                item({ id: 'flour', normalizedMass: 100 }),
                item({ id: 'whipped-cream', normalizedMass: 50, modifiers: ['optional'] }),
            ],
            database
        );
