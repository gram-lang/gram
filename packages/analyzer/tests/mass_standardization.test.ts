...
describe("parseDensityOverrides", () => {
	it('parses a list of "id:density" strings', () => {
		const overrides = parseDensityOverrides({
			densities: ["water:1.0", "honey:1.42"],
		});
	...
	it("ignores malformed entries", () => {
		const overrides = parseDensityOverrides({
			densities: ["not-a-valid-entry", "water:abc", "honey:1.42"],
		});
...
describe("applyYield", () => {
	...
	it("is a no-op when there is no yield factor", () => {
		expect(applyYield(150, "unit_weight", undefined)).toEqual({
			normalizedMass: 150,
		});
	});
