...
function getAliasIndex(database: Record<string, IngredientData>): Map<string, string> {
 ...
 */
export function resolveCanonicalId(name: string, database: Record<string, IngredientData>): string {
    const slug = slugify(name);
