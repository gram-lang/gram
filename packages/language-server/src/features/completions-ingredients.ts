...
export function provideIngredientCompletions(
	db: IngredientDB,
): CompletionItem[] {
	if (Object.keys(db).length === 0) return [];

	return allIngredientCompletionLabels(db).map(
		({ label, canonical, entry }) => {
			...
			if (entry.nutrition) {
				...
				lines.push(
					`${n.calories} kcal/100g · P: ${n.protein}g · C: ${n.carbs}g · F: ${n.fat}g`,
				);
			}
			return {
				label,
