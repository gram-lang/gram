...
export function findAllNameLocations(state: DocumentState, name: string): Location[] {
    if (!state.ast) return [];
    const locations: Location[] = [];

    for (const { decl } of collectIntermediates(state.ast)) {
        if (decl.name === name && decl.loc) {
            locations.push({ uri: '', range: locToRange(state.lineStarts, decl.loc) });
        }
    }
...
export function provideReferences(state: DocumentState, position: Position): Location[] {
    if (!state.ast) return [];
