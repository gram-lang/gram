...
Gram scales the whole base to match how much of it you actually ask for — `&pate{250g}` against a base that measures out to 500g halves it automatically, no `--scale` flag needed. It works out that base's own mass from its ingredients, and tells you when that measurement is only an estimate.
...
**Fixed:** ingredients rescaled through a `@use` import — say, asking for `250g` of a base that naturally yields 553g — used to display at Gram's full internal precision, like `135.62g` of flour, instead of something you'd actually measure out. Quantities in grams, kilograms, millilitres, litres, and similar units now round to one decimal place for display wherever ingredients are shown.
