...
- **@gram/cli**: `--scale id=value` now supports unit conversion (e.g. `flour=1kg` against a recipe in grams), suggests the closest matching ingredient name on a typo, and no longer shows corrupted comparison rows for an ingredient split across multiple units in `gram scale`.